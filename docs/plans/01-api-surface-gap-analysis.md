...
extract and (b) `async-openai` 0.41.1 types. Planning only.

Reference shorthand used below:

| Alias | Path |
| --- | --- |
| `SPEC` | `docs/spec/chat-completions.openapi.yaml`, generated from upstream commit `5c044be3bf3a` (2026-07-23). This tracked extract has stable line numbers; see its header, `openapi.provenance.json`, and `docs/spec/upstream-openapi.md`. |
| `FULL_SPEC` | The untracked `openapi.yaml`, recreated by `scripts/fetch-openapi.ps1` when an out-of-scope path is needed. Its line numbers below refer to commit `5c044be3bf3a`. |
| `AO` | `%USERPROFILE%\.cargo\registry\src\index.crates.io-1949cf8c6b5b557f\async-openai-0.41.1\src` |
| `AO_CHAT` | `AO\types\chat\chat_.rs` |

Anchors: `SPEC:2623` CreateChatCompletionRequest, `SPEC:2909` CreateChatCompletionResponse,
`SPEC:3046` CreateChatCompletionStreamResponse, `SPEC:2299` ChatCompletionResponseMessage,
`SPEC:2440` ChatCompletionStreamResponseDelta, `SPEC:1780` ChatCompletionMessageToolCallChunk,
`SPEC:2411` ChatCompletionStreamOptions, `SPEC:3268` Error, `SPEC:3288` ErrorResponse,
`SPEC:3318` ListModelsResponse, `SPEC:3347` Model, `SPEC:3848` ServiceTier.
...
| `stream_options.include_obfuscation` | `SPEC:2429`, `AO_CHAT:1000` | must be *accepted* (currently ignored, which is acceptable behaviour, but see 3.6) |
...
| `service_tier` (`:100`) | `String` | `auto\|default\|flex\|scale\|priority` (`SPEC:3848`) | echoed to response -> breaks typed clients (see 2.2) |
...
`SPEC:2909-3005` allows exactly `id, choices, created, model, service_tier, system_fingerprint,
...
2. Wrong endpoint: `SPEC:19-134` (the `listChatCompletions` operation and example) shows that the *stored*
...
| `message.annotations` missing | `SPEC:2315` | GUIs that render web-search citations show nothing |
...
than omitted (`SPEC:2921-2925` requires the key), and the `ChatCompletionList` / `ChatCompletionDeleted`
envelopes.
...
| --- | --- | --- | --- |
| 3.1 | First chunk is role-only: `delta:{"role":"assistant","content":""}`, then content chunks (`SPEC:505-511` example) | role folded into the first *content* chunk (`routes.rs:135-140`, `delta_for_text(.., index == 0)` at `service.rs:428`) | cosmetic for most GUIs; fix while touching this code |
| 3.2 | Tool calls stream as `ChatCompletionMessageToolCallChunk`: `index` **required** (`SPEC:1780-1804`, `AO_CHAT:1123` `pub index: u32`), `id`+`type` only on the first fragment, `function.arguments` split across chunks | one final chunk carrying the whole `ChatCompletionMessageToolCall` with **no `index`** (`routes.rs:187`) | HARD BREAK: `serde` reports missing field `index`; `async-openai` stream parsing fails. Unexercised because `tests/async_openai.rs:87-101` only tests tool calls non-streamed |
...
| 3.4 | Final chunk is `delta:{}` + `finish_reason` (`SPEC:511`) | final chunk may also carry content / tool_calls / refusal / audio (`routes.rs:170-190`) | med: double-render risk in GUIs that append every delta |
| 3.5 | Chunk has `service_tier` (`SPEC:3111`); delta has no `audio` field (`SPEC:2440-2477`) | chunk lacks `service_tier` (`model.rs:331-341`); delta emits `audio` (`model.rs:366`) | low both ways; `Option` fields make AO tolerant |
| 3.6 | `obfuscation` is **not** part of the documented chunk schema (in the full spec it appears on Realtime/Responses delta events, e.g. `FULL_SPEC:48641`); only the request flag `stream_options.include_obfuscation` is documented (`SPEC:2429`) | flag ignored, field not emitted | acceptable as-is. Do not invent an `obfuscation` field; optionally add it behind a config flag for GUIs that were written against live traffic |
...
Required shape (`SPEC:3268-3294`): `{"error":{"message":str,"type":str,"param":str|null,
...
| --- | --- | --- |
| `GET /v1/models` | `SPEC:1264` -> `ListModelsResponse` `{object:"list",data:[Model]}` (`SPEC:3318`, `AO\types\models\model.rs:17`) | **In scope, highest endpoint value.** Model pickers in Open WebUI / LibreChat / Chatbox-style GUIs populate from here; without it they show an empty dropdown or refuse to start |
| `GET /v1/models/{model}` | `SPEC:1340` -> `Model` `{id,object:"model",created,owned_by}` (`SPEC:3347`) | **In scope, trivial once the list exists**; unknown id must be 404 `model_not_found` |
| `GET /health` (non-OpenAI) | - | **In scope**: needed by docker/compose healthchecks and CI wait-loops |
| `POST /v1/responses` | `FULL_SPEC:17087` | **Out of short-term scope.** Its streaming surface is dozens of typed events (`response.output_text.delta`, `response.function_call_arguments.delta`, ...) - a multi-week port with its own state model. Only newer GUIs default to it, and most still support Chat Completions. Revisit only if a target GUI is Responses-only |
| `POST /v1/embeddings` | `FULL_SPEC:4418` | **Optional S.** Deterministic pseudo-vector (hash -> normalized floats) is ~60 lines and unblocks GUIs with local RAG/"memory" features. Not needed for chat testing; do it only if a target GUI calls it at startup |
| `POST /v1/moderations` | `FULL_SPEC:8874` | **Optional S.** Some GUIs pre-flight user input. Static "not flagged" response with all category scores 0 is cheap |
...
- [ ] **B2. Split lean POST response from enriched stored object**
  ...
  - Why: removes request-state leakage and stops strict clients rejecting the create response;
    aligns list/get with `SPEC:19-134`.

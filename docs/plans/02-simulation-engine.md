...
the tracked `docs/spec/chat-completions.openapi.yaml` extract (upstream commit `5c044be3bf3a`);
`tiktoken-rs-0.12.0/src/patched_tiktoken.rs`.
...
- Echo `seed` back and derive `system_fingerprint` from the plan, e.g.
  ...
  companion to `seed` (`docs/spec/chat-completions.openapi.yaml:2841-2850`, and the response field is
  marked `deprecated: true` at `:2978-2984`), so a plan-derived value is both

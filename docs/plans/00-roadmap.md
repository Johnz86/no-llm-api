...
| R45 | Doc reconciliation: delete `GEMINI.md`, archive `task.md`, promote `chat_completions_scope.md` to `docs/spec/`, fix two stale `AGENTS.md` claims, add the testing-layout rule, README env/CLI table (merged) | 05#11, 04#10 | S | M5 | R24 |
| R46 | OpenAPI slimming: track a pruned chat-completions extract, untrack `openapi.yaml`, `scripts/fetch-openapi.*` with pinned sha256, drop `openai-func-enums/` (see decision D10) | 05#12 | S | M5 | - |
...
| D8 | Vendored `./async-openai` clone as the reference. 01 and 03 cite it; 05 s6 shows it is version 0.30.1 while we depend on 0.41.1. | (a) keep (b) pin to v0.41.1 (c) use the cargo registry copy | A skewed reference clone is worse than none - it invents constraints the real client does not have. | (c) primary: `~/.cargo/registry/src/*/async-openai-0.41.1/` is guaranteed to match `Cargo.lock`. Optionally keep a pinned clone via `scripts/vendor-refs.*`. Record the decision in `AGENTS.md`. Note plan 01 already used the registry path; plan 03 used the clone. |
...
| D10 | Spec citations point at `openapi.documented.yml`, which 03 verified is **not tracked** (`.gitignore:14`); only the 1.31 MB `openapi.yaml` is. Plans 01 and 04 cite `openapi.documented.yml` line numbers. | (a) track the big file (b) track a pruned extract (c) leave it | Line-number citations into an untracked 2.2 MB file cannot be checked by anyone who clones the repo, which quietly rots every acceptance criterion that references a schema. | (b) per R46: track `docs/spec/chat-completions.openapi.yaml` (pruned, <150 KB) and re-anchor citations to schema **names** plus that file. Do this early in M5 or the citations decay further. |
